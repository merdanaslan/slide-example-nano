document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('beforeprint', () => {
        ...
        slides.forEach((slide, idx) => {
            ...
            clone.style.cssText = 'break-after:page; page-break-after:always; width:100vw; height:100vh; display:flex; flex-direction:column; background:#F0F2F5; border-radius:0;';
            clone.innerHTML = `
                <div style="padding:28px 50px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.6); font-family:'Inter',sans-serif; font-size:0.75rem; font-weight:500; color:#6E6E73;">
                    <span>SUPERTEAM GERMANY</span><span>${(idx+1).toString().padStart(2,'0')} / ${slides.length.toString().padStart(2,'0')}</span>
                </div>
                <div style="flex-grow:1; position:relative;"><div style="position:absolute; inset:0; padding:60px 100px; display:flex; flex-direction:column; justify-content:center;">${slide.innerHTML}</div></div>`;
